...
它是 [T136 的 η/u 宿主关闭](rotation_orbit_five_coloring.md)之后，
...
当 r=1，[T129](valued_rotation_orbits.md)的有限状态有圈论证
...
[T129 §9](valued_rotation_orbits.md#9-已取得并归档的完整指数上界单位边9同点返回6)
...
T129 已核验其唯一提升；本次判断“单位”只需 mod2 的非零见证。
...
τ在所有2处均为单位，所以 T129 原来的2处论证不变：单位接触
...
## 7. E109：完整 rank2 自轨道扫描没有新增动作核障碍

在等待冻结源码的全回归期间，执行了一个有界的新检验：不编译全部
代表对，只检查 H′Y 的**全部自轨道单位接触**。第6节及 T129 已证
...
## 8. 主线接续：Q011 已由 T138 正解决

上文“完整表尚未生成”是来源分支的阶段状态。现在全部点识别、跨轨道接触与五染
已经由另一份16维独立检查闭合，见 [T138/E110](rank2_rotation_five_coloring.md)。
完整接触只有11261条边轨道；不再以增加同一组η/u/τ的层数寻找NON5。

### 候选数的标准 Newton 精化

结合主线 T129 的三项最低阶判据，上文3^r可直接精化为2^r：
每处若C=0或2c_i≥a_i+b_i，则s_i=(b_i−a_i)/2；
若2c_i<a_i+b_i，则s_i∈{c_i−a_i,b_i−c_i}。
否则三项中恰有一个最低阶，不能相消。各处至多两个值，V可逆后至多2^r个向量。
只保留整数并精确代回，实际复二次式仍至多两根。这是已有一维 Newton 规则的逐处应用，
不是全局色数结论；本轮855项有限矩形的完全检查不依赖此优化。
