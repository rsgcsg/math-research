...
2026-09-26 按用户最新指令恢复研究；完整旋转宿主已决定，正在换线。
以下是研究checkpoint，不是目标完成。
...
- [完整 η/u 旋转宿主恰五色](docs/proofs/rotation_orbit_five_coloring.md)（T127/E097）：10076个非零点归为4176轨道；独立穷尽22377条接触与60个原点邻轨，周期3和4五色词直接核验。最小正 u 周期恰3。任意更多正负旋转层均不可能给NON5，E095交换方格的完整诱导图也因此恰五色。
- 已立即检查推广边界：τ将点对32/36映到6268/6256，两份新词均由同色变异色；任意混合及全局重命名仍不能修复τ joint。下一具体对象是独立双生成元 `〈η,u,τ〉`，而不是继续扩旧旋转层。
- [满秩赋值编译](docs/proofs/valuation_rank_contact_compiler.md)（T128）已继续推进这个新对象：两个有限处给独立性；完整Y的两份5进实现证明接触指数仅需 `|n|≤9, |k|≤4`，识别仅需 `|n|≤6, |k|≤2`。855/325个gain是严格完整范围，不是试验截断；尚无新宿主的全边表或五染结论。
- 已立即执行最小新检验（E098）：1259种非零半径的全部自轨道接触仍仅为 `u^±2`，τ没有新增自边。七类交换调色板候选全部保留；下一项须查跨轨道关系，而不是继续增加自接触窗口。这不证明新宿主可五染。

同日上一轮结果：

- [真实旋转轨道的有限程定理](docs/proofs/valued_rotation_orbits.md)（T125）：任意代数无限阶单位旋转与有限代数种子，其全部实际单位边及同点识别可有效穷尽；整个轨道可 k 染当且仅当存在周期染色。有限处赋值与无限处严格外包统一覆盖，不假设原始平面染色周期。
- 主候选 Y 的完整 η 饱和已有严格范围：非零点单位接触只可能在 u 的 −9 至 9 次偏移，同点返回只可能在 −6 至 6 次。T127现已完成全部关系表与无限正染色，最大实际接触偏移为4。
- [全部有理旋转仍恰四色](docs/proofs/rational_rotation_escape_ceiling.md)（T126/E096）：九点非 CM 例的全部有理旋转饱和分解成16点29边的互不相接块。全部有理接触角经精确消元仅为 ±1；停止靠此例增加旋转数量、分母或方向密度寻找升色。
- [真实 η/u 交换方格](docs/proofs/dyadic_commuting_patch.md)（E095）：29669个精确去重点、149822条运输单位边；当时的有界自由五染查询为 UNKNOWN，440109946个潜在点对未全查。该次检查本身不决定诱导图色数；现由T127证明完整诱导图恰五色，旧查询状态不改写。

HY 的完整五染仍不包含旧14运动的全部平移轨道，不能替代 full-joint 或全平面终局。当前转向 τ 的真实联合：先关闭新的几何量词，再处理所有合法颜色关系。验证收据见 [CURRENT](docs/CURRENT.md)。
...
UNKNOWN，第16域未执行。此后的 E085/E086 改查全词定价和指定循环，
并未修复全部15域。E089只关闭固定旧表示，E092只解决u/u²子问题；
自由15域查询仍是历史UNKNOWN，不以重复预算代替新结构。

整体第一路线仍是完整真实颜色关系／全词分离，当前具体入口收敛到
T127之后真正超出旧旋转宿主的完整关系；几何备线须通过真正域外临界核筛选，
上界须越过T122的共尾量词。不再追加已关闭宿主的搜索预算。
