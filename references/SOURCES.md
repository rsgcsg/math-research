...
- [Currie–Eggleton 1509.03667v1 §1](https://arxiv.org/html/1509.03667v1)：
  读取Theorem1及奇轮、三角格分量证明。用于识别正厚度与零厚度量词；
  T097另给本地固定规模紧致性证明，不采用其历史HN四色下界。
- [Atserias–Dalmau 2107.05886v1 §6.1 Theorem4](https://arxiv.org/html/2107.05886v1#S6.SS1)：
  阅读命题及其归约证明；一般图PCSP宽度限制不能外推为平面单位图限制。
- [Chung–Krebs 2605.27778v1 §2–3](https://arxiv.org/html/2605.27778v1)：
  读取Mycielski圈的嵌入维数证明，特别保留单射假设。T098的七点
  同态排除由本地八种圆交选择另证，允许碰撞，不冒用嵌入维数结论。
- 再读[Haugland 2608.04542v4 §1](https://arxiv.org/html/2608.04542v4#S1)：
  spindle-free路线提供模板非必要性的外部证据；本轮不重放其计算。
  Parts2259边证书仍沿用下列固定提交及本地独立检查，不当作本轮新纪录。

具体结论与非结论见[近单位见证及复杂度](../docs/proofs/near_unit_obstruction_compactness.md)。

2026-09-11 T090：重读[Madore 1509.07023v1 ¶2.2及¶1.3](https://arxiv.org/html/1509.07023v1)
的环同态保单位、有限染色紧致性。本地纯超越专门化推论另给完整
证明，不宣称原文已陈述它；该文历史下界不作为当前HN状态。

既有综合检查日期：2026-09-10；后续重读日期另列。原论文结论、作者的计算声明、本地独立验证分别记录。
...
2026-09-11 E061方法收敛复核（不是新增外部定理或重复登记旧证书）：

- [Dúcz–Varga 2606.28157v1](https://arxiv.org/html/2606.28157v1)：
  重读§§4–6；尤其区分极点筛选与整个凸混合的最终有理分离。
  G29此前独立重放仍见原条目；不能把fractional阈值4换成5。
- [Mundinger等2404.05509v1](https://arxiv.org/html/2404.05509v1)：
  重读Theorem1.1与§§2–4，核对(1,1,1,1,1,d)的范围；没有本地重放
  所有铺砌和边界细节，不能从标题“six-colorings”报告普通上界6。
