...
2026-09-26新增核对见[本轮收敛](#sep26)；原始版本和阅读范围见
[SOURCES](SOURCES.md)。单旋转轨道的赋值／无限处增长→SFT连接已得到
本地完整证明，不只是文献类比；T127已将具体无限旋转宿主决定为五色。
普通HN界未变，研究按用户最新要求继续。

2026-09-26接续：已读 Abrishami–Esperet–Giocanti–Hamann–Knappe–Möller
[2411.01951v2 §5.1、Theorem 5.4及证明](https://arxiv.org/html/2411.01951v2#S5)。
这把T125的周期性部分放回virtually-Z SFT／二端拟传递图的成熟背景，
不宣称该一般机制为新发现。原文的连通、局部有限假设须保留；本项目
固定原点的无限星先转为名单，使用自己的有限状态证明。
原文不提供小周期或5／6色自动延拓，尤其不许可搬到两个独立生成元。
真正的新证据是T127的完整算术接触表与可独立核验的无限五染。
...
## 2026-09-26：从真实接触到有限状态；新论文与形式化声称的边界

[T125](../docs/proofs/valued_rotation_orbits.md)把number-field arithmetic、
整数voltage graph和一维SFT真正接起来：对单个代数无限阶旋转，
有限处赋值的唯一最小项不能相消；所有有限处都是单位时，Kronecker
论证保证另一个嵌入有指数增长。两者都穷尽真实接触与同点返回，随后
才可用有限状态有圈判定。当前Y已给完整搜索范围±9／±6，尚未给完整
边表。这里的周期完备性不迁移到一般平移／多独立旋转或整个平面。

[T126/E096](../docs/proofs/rational_rotation_escape_ceiling.md)进一步表明
“一个种子＋稠密角群”可能只是在复制有限图：九点例全部有理角的
接触及相等系统，按数域基比较系数后只允许±1。整个饱和恰四色，
不是固定角词库失败。下一对象若沿用该种子，必须改变接触域或耦合
方式；不再从无限方向、非CM或高度增长推测颜色复杂度。

对近期原文的补充核对也没有改变这个结论。
[Haugland v4](https://arxiv.org/html/2608.04542v4)的无spindle五色图说明
经典核心并非唯一；§4有限四色样本仍没有整个格的上界证明。
[Didin–Voronov v1](https://arxiv.org/html/2607.19946v1)研究每个色类
处处稠密的额外要求，给χ_ed(Rⁿ)≤nχ(Rⁿ)+1及平面≤12。它以稠密
代数独立中心、删除单位球面再处理例外交点构造染色；不是普通六染。
该额外稠密量词也不能直接通过普通有限图紧致性处理。

其“q份独立集二重覆盖＋一个例外色”模板亦不提供≤6捷径：任何有限
单位图限制后二重覆盖给分数染色权重每份1/2，故χ_f≤q/2。已有严格
χ_f>4的有限证据迫使整数q≥9，此模板至少使用10色。这个简单推论
只限该模板，不排除其他非可测／拓扑构造。

另对[固定Lean源码](https://raw.githubusercontent.com/AEjonanonymous/Hadwiger-Nelson/b3366b52fe56d9ea55242b1b41cca5aab70b4f34/Hadwiger_Nelson_Final.lean)
的“χ=7”声称作前提审查，未编译。`SafeDensity`被定义为独立集在圆上
角测度小于π/3，最终`coloring_collision`却把它放在`h_safe`假设中。
该无条件密度命题本身为假：r=1/√3时取半开圆弧
S={r(cos t,sin t):0≤t<2π/3}。任意两点角差δ<2π/3，弦长
2r sin(δ/2)<1，故S独立，而源码定义下的角测度为2π/3。
这不是Lean内核出错，而是所陈述的条件命题没有证明HN所需前提。
不把形式化文件名、条件碰撞不等式或未编译文本升级为公开问题解答。

本轮发散后的实际收敛是：保存完整真实关系优先于抽象匹配算子；
可严格关闭的整族立即退休；剩余第一入口是Y的有限offset全表与可验
周期证书，尚无普通NON5或全局≤6。按用户要求，不再扩展下一轮研究。
