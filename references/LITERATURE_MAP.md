...
## 2026-09-26 本轮原始来源复核与迁移范围

- Haugland，arXiv:2608.04542：本轮作者页为2026-08-06 v2，无spindle的2131点五色例；不改变普通5/7界。
- Abrishami–Esperet–Giocanti–Hamann–Knappe–Möller，arXiv:2411.01951：作者页最新为2025-06-17 v3；此前本轮读取的v2不是最新版。已读作者摘要和所注明HTML范围，局部有限、拟传递、有界pathwidth条件不能省略。这里原点是无限度顶点，先单列名单；新rank2结果由显式正词证明，不引用一般周期存在。
- Krebs–Leyva，arXiv:2511.03028：本轮只读作者摘要，rank指Heuberger矩阵列数，不是两个乘法旋转生成元；不将其染色公式套用于H′Y。

[T138/E110](../docs/proofs/rank2_rotation_five_coloring.md)采用自足赋值范围和精确有限证据，不依赖上述文献给未核验的rank2判定。
两条分支的旧阅读声明只按原身份保留，来源提交482b530…可恢复其原始引用列表。


## 2026-09-21 收敛复核：本次读取与继承记录分开

本次重新读取de Grey 1804.02385、Parts 2010.12665、Haugland 2608.04542v2、
Dúcz–Varga 2606.28157和Sokolov–Voronov 2502.01958的作者页及注明HTML范围。
普通5/7界、图最小化、无spindle五色、分数色数>4、带地图条件的七色是不同命题。
不将旧地图所有“已读”条目升级为本次全文复核，也不复用失效网页引文标记。
本次新增是状态归并和证据重放，不是外部新数学结论；来源见[SOURCES](SOURCES.md)末节。

## 2026-09-21：从跨领域发散到有限坐标分离的实际收敛

本轮有效范围见 [T133–134/E103–104](../docs/proofs/finite_frame_separation.md)，
原文版本和阅读层级见 [SOURCES](SOURCES.md)。普通HN仍无本轮的新界；
本文没有把搜索标题、条件证明或一般复杂性结果当作终局证据。

| 共同结构／领域 | 迁移后真正得到的内容 | 仍不能跨越的假设 |
|---|---|---|
| 图同态／几何承诺CSP | 原问题是所有有限G→U(R²)是否也G→K_k；上下界共享同一完整颜色关系 | 有限restriction的复杂性二分法不直接适用于无限U(R²)；一般模板难不能给二维实现 |
| 有限逆自动机／自由群／有限覆盖 | E103计算完整闭路语言的有限商，实际分开两个提升变量；T133用部分置换完成给统一分离 | 自动机只保等式，恰一色与单位边仍可恢复同样颜色强迫 |
| 代数群／数论／剩余作用 | E104使10077点在模5701双坐标中单射；几何半仿射群与S₄颜色因子给导出长度3的可解单值群 | 坐标分离不是五染色；群类别依赖参考颜色作用，三点实际三角形即能改变它 |
| 概率／因子图／图覆盖 | 完整joint是proper色词上的概率律及完整限制推前相等；覆盖正规形保留这些量词 | 不能将整个Y的proper因子替换为相互独立的小边际；Bethe类比不是可行性证明 |
| 动力系统／有限模型 | 已有T129–130把真实单旋转全部返回与接触压为有限记忆系统并给整个轨道五色 | 多运动共同不变律仍缺；一维周期存在性不能无证据推广为多生成元周期性 |
| 谱／调和／拓扑／逻辑 | 独立比、指定coindex、有限验证各有清晰作用；外部χ=7代码审计指出密度前提未证明 | 单色或正则性受限指标不等于普通色数；形式化条件命题不等于证明条件 |

**本轮实际反驳了自身的候选机制。** 新S₃覆盖通过全24235点组件的分离，
却在完整proper约束下被195步单位推导排除。因此不把“非幂零”或“类分裂”
当作足够进展条件。T133–134进一步说明纯等式坍缩不能普遍封闭覆盖正规形。
要继续产生下界，必须给对全部proper色词成立的联合关系；要继续产生上界，
必须保留所有实际边和全域／共尾量词。

**文献状态更新。** de Grey是普通五色下界；Haugland当前可读v2为无spindle
五色构造；Dúcz–Varga为独立比<1/4；map-type七色结果带边界和局部有限假设。
它们是不同指标或不同承诺。Haugland旧v4阅读记录本轮未复核，以下历史细节
不得由本次v2页面再次背书。一个搜索命中的“Lean证明χ=7”声明已直接检查源文件，
关键SafeDensity为假设，且通用密度命题有半径圆／半开弧反例，未改变普通界。
...
6. **Haugland（以下为历史v4记录，本轮仅复核到作者页v2，不重复认证以下v4细节），A Moser-spindle-free 5-chromatic unit distance graph
...
## 2026-09-16收敛：有限对称补全、循环流与广义Paley

[EPPA numbers of graphs, 2311.07995v2](https://arxiv.org/html/2311.07995v2)
提供成熟的部分自动同构补全语言。关键不是能否有限补全，而是能否
**保持k可染**。本地[T120](../docs/proofs/finite_joint_completion.md)
证明这恰与固定有限full-joint可行等价；构造用分类积K_k与重复码列的
独立纤维副本，不要求二维实现或几何群关系。C023四点实际P₄已否定
一般保色数版本，并用第五点编译实际C₅。原文一般图的EPPA规模结论
不能变成HN最小non-5见证规模或证明复杂度下界。

概率边际在一个运动下也有更简单语言：每份完整proper词是一条
“源划分→像划分”的边，有限词池可行当且仅当有向图含环。
[E085/E086](../docs/proofs/dyadic_joint_column_pricing.md) 实际验证了这个
区别：全词定价降到−1仍只延长一条无环路径；想闭合的第三边因物理
交叠上的两点划分矛盾而不存在。它与一维Markov循环流结构相近，
但多运动共同权重、稀疏几何交叠和实际单位边仍是额外条件。
没有把单运动有环或一个cut改善说成旧14域共同可延拓。

[Schneider–Silva, 1306.6050v1](https://arxiv.org/pdf/1306.6050v1) 的
广义Paley定义准确容纳H29：范数一子群阶30、指数28，图是Γ_(841,28)。
平方阶的普通Paley色数公式依赖指数整除30，这里不满足。
[T121](../docs/proofs/residue29_rotation_ceiling.md) 的颜色作用必须
忠实且固定原点，故完整旋转单词表示≥11色；普通染色或不变概率律
没有该假设。E087未获得非线性六色正词，停止固定表示预算。

来源审核补强而不是改变T053：Vinh两份早稿的√q常数被D5精确谱例
和D7完整四色直接反驳；Harcos Theorem1的2√p核对覆盖29，Conrad
仍提供一般素数幂输入。详见[版本与阅读范围](residue29_structure_review_20260916.md)。
不允许用错误常数删去29，更不允许把有限目标失败反推数域或HN下界。

本轮没有普通HN新界。最有信息量的续研对象是保留全模式行、源像
交叠相容及旧义务的共同流，或对全部proper词有效的严格分离；
“一般保色数EPPA”已被小例否定，不再作为无条件捷径。上一次已按
用户要求停止；现按新指令恢复，以下记录新增的收敛与实际构造。

## 十一、恢复研究：有限单位同态、域共尾与真正的新颜色关系

[Tyszka的有限Beckman–Quarles原文](https://arxiv.org/html/math/9904047v2)
使问题从“坐标是否属于某类域”提升为一个刚性不变量：任意正实代数
平方距离都能由有限单位约束强迫。[T122](../docs/proofs/unit_homomorphism_cofinality.md)
据此证明实域平方族的有限同态普适性与域包含共尾等价，修正E082原先
保留的较弱捷径。这不是重新命名共尾，而是反向量词的严格证明。

随后没有停在外部存在性：[T123/E091](../docs/proofs/cm_escape_nine.md)
实际压出九点反例。Moser七点加两个点便能排除全部CM单位同态，
仍然χ4、原七端口的k色关系完全不变。其平方距离的另一实共轭大于4，
而实际两单位边路径强制≤4；这说明顺序／正定性而非“根式数量”
才是几何逃逸机制。九点不宣称最小，经典杆也不宣称首创。

[Rabern v5 Theorem2](https://arxiv.org/html/1205.3253v5) 的列表Brooks
进一步防止过拟合：域外点至多接两个旧域点，最大内度3的整个新层
都可延拓任意旧五染。non-5逃域核必须有3-core且出现4度分支；
non-6对应4-core和5度分支。这仍只是必要条件，不提供激活或坏名单。

动力系统连接同样产生实际信息：[E090](../docs/proofs/dyadic_return_boundary.md)
把单运动joint写为有限状态循环流，但两点三角轨道反例显示一步strip
遗漏真实回入。当前u²有800个这种跳回。[E092](../docs/proofs/dyadic_identity_extension.md)
已实际在整个Y上同时修复u/u²身份作用；新词越过旧22模式DAG，
但旧14域仍未共同修复。SFT语言解释了量词，而没有自动制造障碍。

本次因此仍将第一优先放在**新算子、新模式且保旧义务的共同律**，
或全proper严格分离；停止旧表示微调、几何对称免费扩包和独立逃域点。
上界只能寻求真共尾／零坏边极限，或更弱但尚未证明的色数极值归约。

继续向远处发散时，[Radchenko的Salem整数构造](https://arxiv.org/html/1807.03726v1)
给了一个容易误判的候选：有限秩四加法群已有无限单位方向，而且
不能放回CM有限生成模块。没有停在这个漂亮几何上；实际计算五点
全部十对并构造显式3进赋值后，[T124](../docs/proofs/salem_field_three_coloring.md)
证明整个四次数域恰三染，所有分母一并覆盖。一个共轭稳定、剩余
作用平凡的素点便把单位方向压成±1；剩余图只是奇圈或双向路。
因此“真实非CM／无限valency”与“跨越颜色天花板”仍是两回事。
该具体宿主不再扩大搜索，下一步重新回到多运动共同律及实际临界核。
随后并未只留下“试不分歧域”的建议：T124 §8实际检验了
p₆=X⁶−X⁴−X³−X²+1。迹对偶和已核读Milne判别式判据证明其K/F
在所有有限处不分歧，显式2进赋值仍给整个K四染。它只确定3≤χ≤4，
说明消除上一类三色天花板还不够，不能把必要条件反过来当升色迹象。

这种“组合两个正例”的期待随后也被实测结构筛选：
[E093](../docs/proofs/operator_release.md) 的旧词与新词颜色对，沿全部
实际单位边投影后恰为完整K₅×K₅。分类积的五染只能是坐标投影，
所以任意点无关五色合并没有新划分；一个完整模式势进一步排除这些
输出的非负混合。这是图同态语言实际增加信息的例子，证明在项目中
完整给出，不依赖一般CSP分类，也不声称超过固定词对的普适结论。

[E094](../docs/proofs/dyadic_eta_release.md)没有停在“释放η试试”的
路线图：实际自由查询后，百万行原轨迹被同公式的13步RUP替代并
独立核验。1081子句核心不引用η义务，严格关闭全部单算子修补。
这体现证明复杂度与搜索状态的区别：短受限反证仍不是HN下界，
不再以更多同型局部修补消耗主线预算。


## 小支持完整律：2026-09-20接入trade理论

Ghorbani–Kamali–Khosrovshahi–Krotov的二进制[t]-trade（含重数）把
“低阶矩一致但完整分布不同”变成整数抵消系统。已核读的最小体积乘积
结构与二阶体积5空隙，结合划分传递性，导出本仓库[T127](../docs/proofs/five_word_quartet_completeness.md)：
同m≤5、等权、≤5颜色的完整律由二次相等矩精确决定。六颜色奇偶开关
解释颜色假设为什么特殊；六词反例解释词数为什么特殊。
任意权的五支持版本另由有限支持覆盖反证得到尖锐五点T128，不能用T127替代。
