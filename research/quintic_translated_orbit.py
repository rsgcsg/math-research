...
def run(root, union=False):
    ...
    def label(p):
        ...
        return 11*x+y
    cases=[]
    specs=(((1,2,3,4,1),0),) if union else (((1,),0),((1,2,3,4),0),((1,),1))
    for extra,offset in specs:
        anchor=one if offset else zero
        anchors=[zero]*4+[one] if union else [anchor]*len(extra)
        bs=blocks+[[add(a,multiply(powers[j-1],p)) for p in shifted] for j,a in zip(extra,anchors)]
        ns=neighbors+[{i:[a,add(a,multiply(add(powers[j-1],conjugate(powers[j-1])),shifted[i]))] for i in units} for j,a in zip(extra,anchors)]
        ...
        interface={zero,*anchors}|{p for nn in ns for pp in nn.values() for p in pp}
        if union:
            left=set(sum(ids[:8],[]));right=set(ids[8]);old_r=set(sum(ids[:4],[]))
            # Edges absent from both separately certified right-hand hosts.
            summary['new_union_edges']=sum(not ({i,v}<=left or {i,v}<=old_r|right) for i,v in edges)
        ...
        with Solver(name='cd19',bootstrap_with=clauses) as solver:
            solver.conf_budget(1000000 if union else 100000);answer=solver.solve_limited()
    ...
    return dict(schema=2 if union else 1,experiments=['E067'] if union else ['E058','E059','E060'],center=64,core_sha256=hashlib.sha256(raw).hexdigest(),
...
if __name__=='__main__':
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--output',type=Path);parser.add_argument('--union',action='store_true');args=parser.parse_args()
    data=run(Path(__file__).resolve().parents[1],union=args.union)
